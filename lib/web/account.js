...
exports.get = function (request) {
    ...
    if (request.params.panel &&
        ...
        var locals = {
            env: {
                ...
                message: request.state.yar.message || ''
            }
        };
    ...
    if (request.state.yar.message) {
        request.plugins.yar.message = request.state.yar.message;
    }
...
exports.profile = function (request) {
    ...
    Api.call('POST', '/profile', body, request.session, function (err, code, payload) {
        ...
        if (err || code !== 200) {
            request.plugins.yar.message = 'Failed saving changes. ' + (code === 400 ? payload.message : 'Service unavailable');
        }
...
exports.emails = function (request) {
    ...
    if (['add', 'remove', 'primary', 'verify'].indexOf(request.payload.action) === -1) {
        request.plugins.yar.message = 'Failed saving changes. Bad request';
    ...
    Api.call('POST', '/profile/email', request.payload, request.session, function (err, code, payload) {
        ...
        if (err || code !== 200) {
            ...
            request.plugins.yar.message = 'Verification email sent. Please check your inbox (or spam folder) for an email from ' + Config.product.name + ' and follow the instructions.';
        }
