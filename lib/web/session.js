...
exports.ask = function (request) {
    ...
    Api.clientCall('GET', '/oz/app/' + request.query.client_id, null, function (err, code, client) {
        ...
        request.plugins.yar.oz = { client: client, redirection: redirectionURI };
        if (request.query.state) {
            request.plugins.yar.oz.state = request.query.state;
        }
...
exports.answer = function (request) {
    ...
    if (!request.state.yar.oz ||
        !request.state.yar.oz.client) {

        return request.reply.redirect('/').send();
    }

    var options = {
        issueTo: request.state.yar.oz.client.id,
        scope: []
    };
    ...
    Api.call('POST', '/oz/reissue', options, request.session, function (err, code, ticket) {
        ...
        if (request.state.yar.oz.state) {
            ticket.state = request.state.yar.oz.state;
        }

        return request.reply.redirect(request.state.yar.oz.redirection + '#' + QueryString.stringify(ticket)).send();
    });
};
