...
YUI.add('postmile-stream', function (Y) {
    ...
    function handleStreamMessage(message) {
        ...
        if (message.type === 'connect') {
            var auth = hawk.client.message(postmile.api.domain, postmile.api.port, message.session, { credentials: session });
    ...
    function handleStreamUpate(message) {
        ...
        if (session.id.lastIndexOf(message.macId) === session.id.length - 8) {
