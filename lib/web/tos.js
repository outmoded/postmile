...
exports.get = function (request) {
    ...
    return request.reply.redirect((request.query.next && request.query.next.charAt(0) === '/' ? request.query.next : request.auth.credentials.profile.view));
};
...
exports.post = function (request) {
    ...
    Api.clientCall('POST', '/user/' + request.auth.credentials.profile.id + '/tos/' + exports.currentTOS, '', function (err, code, payload) {
        ...
        Session.refresh(request, request.auth.credentials, function (err, session) {
            ...
            return request.reply.redirect('/tos' + (request.payload.next ? '?next=' + encodeURIComponent(request.payload.next) : ''));
        });
    });
};
