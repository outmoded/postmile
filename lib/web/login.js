...
exports.auth = function (request) {
    ...
    var entry = function () {
        ...
        if (request.query.x_next &&
            ...
            request.plugins.yar.auth = { next: request.query.x_next };
        }
    ...
    var twitter = function () {
        ...
        if (!request.query.oauth_token) {
            return twitterClient.getOAuthRequestToken(function (err, token, secret, authorizeUri, params) {
                ...
                request.plugins.yar.twitter = { token: token, secret: secret };
    ...
    var facebook = function () {
        ...
        if (!request.query.code) {
            ...
            request.plugins.yar.facebook = { state: request.query.state };
        ...
        if (!request.state.yar.facebook ||
            !request.state.yar.facebook.state) {

            return request.reply(Hapi.error.internal('Missing Facebook state cookie'));
        }

        if (request.state.yar.facebook.state !== request.query.state) {
    ...
    var yahoo = function () {
        ...
        if (!request.query.oauth_token) {
            yahooClient.getOAuthRequestToken(function (err, token, secret, authorizeUri, params) {
                ...
                request.plugins.yar.yahoo = { token: token, secret: secret };
    ...
    var finalizedLogin = function (account) {
        ...
        if (request.session &&
            ...
            var destination = request.state.yar.auth && request.state.yar.auth.next;
...
exports.loginCall = function (type, id, request, destination, account) {
    ...
    Api.clientCall('POST', '/oz/login', payload, function (err, code, payload) {
        ...
        if (code !== 200) {
            ...
            if (type === 'email') {
                request.plugins.yar.message = payload.message;
                return request.reply.redirect('/').send();
            }
            ...
            if (account) {
                request.plugins.yar.signup = account;
            ...
            Session.set(request, ticket, function (isValid, restriction) {
                ...
                if (payload.ext &&
                    ...
                    switch (payload.ext.action.type) {
                        case 'reminder':
                            request.plugins.yar.message = 'You made it in! Now link your account to Facebook, Twitter, or Yahoo! to make sign-in easier next time.';
                            destination = '/account/linked';
                            break;
                        case 'verify':
                            request.plugins.yar.message = 'Email address verified';
