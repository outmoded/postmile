...
exports.create = function () {
    ...
    server.plugin().require('tv', { permissions: { ext: true } }, function (err) {

        Hapi.utils.assert(!err, 'Failed loading tv plugin: ' + err);
        server.plugin().require('yar', { permissions: { ext: true }, plugin: { options: { password: Vault.yar.password }, isSingleUse: true } }, function (err) {

            Hapi.utils.assert(!err, 'Failed loading yar plugin: ' + err);
            server.plugin().require('crumb', { permissions: { ext: true } }, function (err) {

                Hapi.utils.assert(!err, 'Failed loading crumb plugin: ' + err);
                server.plugin().require('scooter', { permissions: { ext: true } }, function (err) {

                    Hapi.utils.assert(!err, 'Failed loading scooter plugin: ' + err);
